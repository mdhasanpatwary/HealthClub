    ...
    <div className={`lg:col-span-5 space-y-3.5 ${mobileTab === "preview" ? "block" : "hidden lg:block"}`}>
        ...
        <div className="flex items-center justify-between border-b border-border/60 pb-2.5">
          ...
          <div className="text-right">
            <span className="text-lg sm:text-xl font-extrabold text-primary font-heading">
              {toBanglaNums(currentReport.overallScore)}
              <span className="text-[10px] font-normal text-muted-foreground">/১০০</span>
            </span>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="p-2 sm:p-2.5 rounded-xl bg-background border border-border/60 space-y-0.5">
            ...
            <div className="font-bold text-xs sm:text-sm text-foreground">
              {toBanglaNums(currentReport.bmi)}{" "}
          ...
          <div className="p-2 sm:p-2.5 rounded-xl bg-background border border-border/60 space-y-0.5">
            ...
            <div className="font-bold text-xs sm:text-sm text-foreground">
              {toBanglaNums(currentReport.maintenanceCalories)}{" "}
          ...
          <div className="p-2 sm:p-2.5 rounded-xl bg-background border border-border/60 space-y-0.5">
            ...
            <div className="font-bold text-xs sm:text-sm text-foreground">
              {toBanglaNums(currentReport.dailyWaterLiters)} লিটার
            </div>
            <p className="text-[9px] text-muted-foreground truncate">
              ~{toBanglaNums(currentReport.dailyGlasses)} গ্লাস
            </p>
          </div>
          ...
          <div className="p-2 sm:p-2.5 rounded-xl bg-background border border-border/60 space-y-0.5">
            ...
            <div className="font-bold text-xs text-foreground">
              {currentReport.bpEvaluation
                ? `${toBanglaNums(currentReport.bpEvaluation.systolic)}/${toBanglaNums(currentReport.bpEvaluation.diastolic)}`
                : "১২০/৮০"}
            </div>
        ...
        <div className="p-2 sm:p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[10.5px] text-emerald-800 dark:text-emerald-300 flex items-start gap-1.5">
          ...
          <div className="min-w-0">
            ...
            <span className="truncate block">
              {currentReport.doctorReferralBn}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="pt-1">
        ...
        >
          ...
          <span>রিপোর্ট প্রিন্ট / PDF সংরক্ষণ করুন</span>
        </Button>
      </div>
    </div>
  );
