        ...
        <div className="print:hidden p-5 sm:p-6 border-b border-border flex items-center justify-between gap-4 bg-muted/40 sticky top-0 z-10 backdrop-blur">
          <div>
            <DialogTitle className="text-base sm:text-lg font-bold font-heading text-secondary dark:text-white flex items-center gap-2">
              ...
              <span>মাসিক সেটেলমেন্ট ও বিলিং স্টেটমেন্ট</span>
            </DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">
              {statement.monthLabelBn} এর পূর্ণাঙ্গ স্টেটমেন্ট ও বিলিং সারসংক্ষেপ
            </DialogDescription>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs">
            <div className="space-y-1">
              <p className="text-slate-500 font-semibold uppercase tracking-wider text-[10px]">
                পার্টনার হাসপাতাল / প্রতিষ্ঠান
              </p>
              <p className="text-sm font-bold text-slate-900">{partner.name}</p>
              <p className="text-slate-600">{partner.address}</p>
              <p className="text-slate-600 font-mono">
                ফোন: {partner.phone} {partner.emergencyPhone && `| জরুরি: ${partner.emergencyPhone}`}
              </p>
            </div>

            <div className="space-y-1 sm:text-right">
              <p className="text-slate-500 font-semibold uppercase tracking-wider text-[10px]">
                বিলিং মেয়াদ / স্টেটমেন্ট মাস
              </p>
              <p className="text-base font-extrabold text-emerald-700">
                {statement.monthLabelBn}
              </p>
              <p className="text-slate-600">
                পার্টনার ডিসকাউন্ট হার:{" "}
                <strong className="text-slate-900">{partner.discount}</strong>
              </p>
              <p className="text-slate-600">
                মোট সেবাগ্রহীতা:{" "}
                <strong className="font-mono text-slate-900">{toBanglaNums(statement.totalTransactions)} জন</strong>
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3.5 rounded-xl border border-slate-200 bg-slate-50/80">
              ...
              <p className="text-lg font-bold font-mono text-slate-800 mt-0.5">
                ৳{toBanglaNums(statement.grossAmount)}
              </p>
            </div>
            <div className="p-3.5 rounded-xl border border-emerald-200 bg-emerald-50/60">
              <p className="text-[10px] uppercase font-bold text-emerald-700">
                মোট ছাড় প্রদান (ডিসকাউন্ট)
              </p>
              <p className="text-lg font-bold font-mono text-emerald-700 mt-0.5">
                ৳{toBanglaNums(statement.totalDiscountDispensed)}
              </p>
            </div>
            <div className="p-3.5 rounded-xl border border-slate-200 bg-slate-50/80">
              <p className="text-[10px] uppercase font-bold text-slate-500">
                রোগী কর্তৃক মোট পরিশোধিত
              </p>
              <p className="text-lg font-bold font-mono text-slate-800 mt-0.5">
                ৳{toBanglaNums(statement.netPatientPaid)}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700">
                আইটেমাইজড রোগী ও লেনদেন তালিকা
              </h3>
              <span className="text-[11px] text-slate-500 font-mono">
                {toBanglaNums(statement.transactions.length)} টি রেকর্ড
              </span>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-slate-100 border-b border-slate-200 text-slate-700 font-bold">
                  <tr>
                    ...
                    <th className="p-2.5">মেম্বার আইডি</th>
                    <th className="p-2.5">রোগীর নাম</th>
                    <th className="p-2.5 text-right">গ্রস বিল</th>
                    <th className="p-2.5 text-right text-emerald-700">ডিসকাউন্ট</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {statement.transactions.map((tx, idx) => (
                    <tr key={tx.id} className="hover:bg-slate-50/50">
                      <td className="p-2.5 text-center text-slate-400 font-mono">{toBanglaNums(idx + 1)}</td>
                      ...
                      <td className="p-2.5 text-right font-mono text-slate-700">৳{toBanglaNums(tx.amount)}</td>
                      <td className="p-2.5 text-right font-mono font-bold text-emerald-600">৳{toBanglaNums(tx.saved)}</td>
                      <td className="p-2.5 text-right font-mono font-semibold text-slate-800">৳{toBanglaNums(tx.amount - tx.saved)}</td>
                    </tr>
                  ))}
                  <tr className="bg-slate-100 font-bold border-t-2 border-slate-300">
                    ...
                    <td className="p-2.5 text-right font-mono text-slate-900">৳{toBanglaNums(statement.grossAmount)}</td>
                    <td className="p-2.5 text-right font-mono text-emerald-700">৳{toBanglaNums(statement.totalDiscountDispensed)}</td>
                    <td className="p-2.5 text-right font-mono text-slate-900">৳{toBanglaNums(statement.netPatientPaid)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="pt-8 grid grid-cols-2 gap-8 text-xs border-t border-slate-200 mt-6">
            <div className="space-y-10">
              ...
              <div className="border-t border-slate-400 w-44 pt-1.5 text-center">
                <p className="font-bold text-slate-800">হাসপাতাল কর্তৃপক্ষ / ক্যাশিয়ার</p>
            ...
            <div className="space-y-10 text-right flex flex-col items-end">
              ...
              <div className="border-t border-slate-400 w-44 pt-1.5 text-center">
                <p className="font-bold text-slate-800">অনুমোদিত কর্মকর্তা</p>
